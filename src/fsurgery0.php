...
CREATE TABLE \"surgeryiitable\" (
				\"surgeryiiid\" serial NOT NULL,                	-- (id de ficha clinica cirugia bucal ii)
        ...
        \"surgeryiiforecast\" varchar(200) DEFAULT '',    -- (pronostico)
				\"surgeryiistatus\" varchar(50) DEFAULT '',       -- (estado new, process, end, fail)
        ...
        CONSTRAINT \"surgeryii_pkey\" PRIMARY KEY (\"surgeryiiid\", \"student\", \"teacher\"),
	...
	$r = DBExec($c, "CREATE INDEX \"surgeryii_index\" ON \"surgeryiitable\" USING btree ".
				"(\"surgeryiiid\" int4_ops, \"patientid\" int4_ops, \"student\" int4_ops)",
...
function DBNewSurgeryii($param , $c=null){
		...
		if(isset($param['surgeryiistatus']) && !isset($param['status'])) $param['status']=$param['surgeryiistatus'];
		...
		'evolution', 'forecast','status','updatetime', 'startdatetime', 'enddatetime');

		$typei['patient']=-1;
		$typei['student']=-1;
		$typei['remission']=-1;
		$typei['teacherid']=-1;//admin
		$typei['clinical']=5;//clinica cirugia bucal II

		$typei['updatetime']=-1;
		$typei['startdatetime']=-1;
		$typei['enddatetime']=-1;
		...
		$sql = "select * from surgeryiitable where patientid=$patient and remissionid= $remission";
		$a = DBGetRow ($sql, 0, $c);
			if($teacher==-1){
				if($a!=null){
					...
					$practice=$a['surgeryiipractice'];
					$motconsult=$a['surgeryiimotconsult'];
					$personalremote=$a['surgeryiipersonalremote'];
					$dentalhistory=$a['surgeryiidentalhistory'];
					$physicalexam=$a['surgeryiiphysicalexam'];
					$odontogram=$a['surgeryiiodontogram'];
					$asa=$a['surgeryiiasa'];
					$anxiety=$a['surgeryiianxiety'];
					$diagnosishypothesis=$a['surgeryiidiagnosishypothesis'];
					$complementaryexam=$a['surgeryiicomplementaryexam'];
					$surgicaldifficulty=$a['surgeryiisurgicaldifficulty'];
					$consent=$a['surgeryiiconsent'];
					$treatmentplan=$a['surgeryiitreatmentplan'];

					$disease=$a['surgeryiidisease'];
					$exam=$a['surgeryiiexam'];
					$diagnosis=$a['surgeryiidiagnosis'];
					$treatment=$a['surgeryiitreatment'];
					$prescriptions=$a['surgeryiiprescriptions'];
					$indications=$a['surgeryiiindications'];
					$evolution=$a['surgeryiievolution'];
					$forecast=$a['surgeryiiforecast'];
					$status=$a['surgeryiistatus'];
				}else{
					$teacher=0;
				}
			}
			...
			if ($a == null) {
	    		...
	    		$sql = "insert into surgeryiitable(surgeryiipractice,	surgeryiimotconsult, surgeryiipersonalremote, ".
					...
					"surgeryiistatus, patientid, student, remissionid, teacher, clinicalid, updatetime) values ".
	    		"('$practice',	'$motconsult',	'$personalremote',	'$dentalhistory',	'$physicalexam', ".
					...
					"'$status', $patient, $student, $remission, $teacher, $clinical, $updatetime)";
				...
				if($updatetime > $a['updatetime']) {
					$ret=2;
					$sql='';
					if($a['clinicalid']==13){//cirugia bucal III de quinto año
						//surgeryiiconsent='$consent',
						$sql = "update surgeryiitable set surgeryiipractice='$practice',surgeryiimotconsult='$motconsult',".
						"surgeryiipersonalremote='$personalremote',surgeryiidentalhistory='$dentalhistory',".
						"surgeryiiphysicalexam='$physicalexam',surgeryiiodontogram='$odontogram',surgeryiiasa='$asa',".
						"surgeryiianxiety='$anxiety',surgeryiidiagnosishypothesis='$diagnosishypothesis',".
						"surgeryiicomplementaryexam='$complementaryexam',surgeryiisurgicaldifficulty='$surgicaldifficulty',".
						"surgeryiitreatmentplan='$treatmentplan',".
						"surgeryiidiagnosis='$diagnosis', ".
						"surgeryiistatus='$status', student=$student, teacher=$teacher, updatetime=$updatetime " .
						"where patientid=$patient and remissionid=$remission";
					}else{
						$sql = "update surgeryiitable set surgeryiidisease='$disease', surgeryiiexam='$exam', surgeryiidiagnosis='$diagnosis', ".
									"surgeryiitreatment='$treatment' ,surgeryiiprescriptions='$prescriptions', surgeryiiindications='$indications', ".
									"surgeryiievolution='$evolution', surgeryiiforecast='$forecast', surgeryiistatus='$status', student=$student, teacher=$teacher, updatetime=$updatetime";
...
function DBSurgeryiiInfo($id, $xremission=false, $c=null) {
	...
	$ob=DBObservationInfo($a['surgeryiiid'],$a['remissionid']);
	if($ob!=null)
		$a=array_merge($a, $ob);
	$a=clearexam($a);
	$a=cleartreatment($a);
